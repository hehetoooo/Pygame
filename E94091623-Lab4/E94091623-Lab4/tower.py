...
class Circle:
    ...
    def collide(self, enemy):
        ...
        x1, y1 = enemy.get_pos()    #get enemys coordinate
        distance = math.sqrt((self.center[0] - x1) ** 2 + (self.center[1] - y1) ** 2)  # calculate the distance between the enemy and the tower
        if distance <= self.radius:  # determine the scope of the attack
            return True
        else:
            return False
...
class Tower:
    ...
    def is_cool_down(self):
        ...
        if self.cd_count < self.cd_max_count: #when  cd  < cd max  --> False
            self.cd_count += 1
            return False
        else:
            self.cd_count = 0  # reset cd  to  0
            return True
    ...
    def attack(self, enemy_group):
        ...
        if self.is_cool_down():  # cool down time
            for enemy in enemy_group.get():
                if self.range_circle.collide(enemy):  # If the enemy is within the attack range
                    enemy.get_hurt(self.damage)     # beat it
                    break   # stop beating after the tower hits the enemy
